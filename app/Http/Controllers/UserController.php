 ...
 *                 @OA\Property(property="tin", type="string", example="123456789")
 *             )
 *         )
 *     ),
 *     @OA\Response(
 ...
 *                 @OA\Property(property="email", type="string", example="john@example.com"),
 *                 @OA\Property(property="image", type="string", example="userimage/user1.jpg"),
 *                 @OA\Property(property="customer_type", type="string", example="company"),
 *                 @OA\Property(property="vat", type="string", example="GB123456789", nullable=true),
 *                 @OA\Property(property="tin", type="string", example="123456789", nullable=true),
 *                 @OA\Property(property="email_verified_at", type="string", format="date-time", nullable=true),
 *                 @OA\Property(property="created_at", type="string", format="date-time"),
 *                 @OA\Property(property="updated_at", type="string", format="date-time")
 *             )
 ...
 *         @OA\JsonContent(
 *             @OA\Property(property="status", type="boolean", example=false),
 *             @OA\Property(property="message", type="string", example="VAT and TIN updates are only allowed for company or employee users")
 *         )
 *     )
 * )
 */
public function updateUser(Request $request)
{
    ...
    // Check if VAT or TIN fields are being updated
    $hasVatOrTin = $request->filled('vat') || $request->filled('tin');
    
    // If user is trying to update VAT or TIN but they're an individual customer type
    if ($hasVatOrTin && $user->customer_type === 'individual') {
        return response()->json([
            'status' => false,
            'message' => 'VAT and TIN updates are only allowed for company or employee users'
        ], 403);
    }
    
    // Add VAT and TIN validation rules if the user is company or employee
    if ($user->customer_type === 'company' || $user->customer_type === 'employee') {
        if ($request->filled('vat')) {
            $rules['vat'] = 'string|max:50';
        }
        
        if ($request->filled('tin')) {
            $rules['tin'] = 'string|max:50';
        }
    }
    ...
    // Update VAT and TIN only if user is company or employee
    if ($user->customer_type === 'company' || $user->customer_type === 'employee') {
        if ($request->filled('vat')) {
